# =============================================================================
# MONITOR DE PRODUCT ADS — MERCADO LIVRE (Versão 11.0 — Design Agência Profissional)
# =============================================================================

import subprocess
subprocess.run(["pip", "install", "requests", "pandas", "plotly", "--quiet"])
...
VERBA_MENSAL   = float(input("📌 Verba mensal desta conta? (ex: 3000): R$ ").strip().replace(",", "."))
DIA_FECHAMENTO = int(input("📌 Dia de fecho desta conta? (ex: 17): ").strip())
print(f"\n✅ Verba: R$ {VERBA_MENSAL:,.2f} | Fecho: todo o dia {DIA_FECHAMENTO}")
print("=" * 60)

# ── AUTENTICAÇÃO ──────────────────────────────────────────────
print("\n🔐 AUTENTICAÇÃO — MERCADO LIVRE\n")
client_id     = input("📌 Cole aqui o seu CLIENT ID: ").strip()
client_secret = input("📌 Cole aqui o seu CLIENT SECRET: ").strip()
...
print(f"\n  👉  {auth_url}\n")
print('Após autorizar, copie o valor do campo "code" da página JSON.')
...
if token_resp.status_code != 200:
    print("❌ ERRO:", token_resp.json())
    sys.exit()

access_token = token_resp.json()["access_token"]
print("✅ Token obtido!")

h1 = {"Authorization": f"Bearer {access_token}", "Api-Version": "1"}
h2 = {"Authorization": f"Bearer {access_token}", "api-version": "2"}
...
adv_lista = adv_resp.json().get("advertisers", [])
if not adv_lista:
    print("❌ Nenhuma conta encontrada.")
    sys.exit()

print(f"✅ Encontrada(s) {len(adv_lista)} conta(s) ativa(s)!")
...
for a in adv_lista:
    ...
    print(f"\n💼 Configurações para a loja: {a_nome}")
    try:
        verba = float(input("  📌 Verba mensal? (ex: 3000): R$ ").strip().replace(",", "."))
    except:
        verba = 3000.0
    try:
        fechamento = int(input("  📌 Dia de fecho? (ex: 17): ").strip())
    except:
        fechamento = 17
    configs_lojas[a_id] = {"verba": verba, "fechamento": fechamento}
...
print("\n📡 A recolher dados das campanhas...")

for a in adv_lista:
    adv_id = a["advertiser_id"]
    adv_nome = a.get("advertiser_name", f"ID: {adv_id}")
    
    verba_mensal = configs_lojas[adv_id]["verba"]
    dia_fechamento = configs_lojas[adv_id]["fechamento"]
...
if all_campaign_records:
    ...
    nome_csv = f"relatorio_master_ads_{hoje.strftime('%Y%m%d')}.csv"
    df_csv.to_csv(nome_csv, index=False, encoding="utf-8-sig", sep=";")
    print(f"\n💾 Relatório CSV Consolidado guardado: {nome_csv}")
    
    try:
        from google.colab import files
        files.download(nome_csv)
        print("⬇️ Download iniciado automaticamente!")
    except Exception as e:
        print("📥 Download não pôde ser iniciado automaticamente (Não rodando no Colab).")
else:
    print("\n⚠️ Nenhuma campanha com dados de ontem para gerar CSV.")
